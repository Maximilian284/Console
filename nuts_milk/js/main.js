    ...
    for(let y = 0; y < map.length; y++){
        for(let x = 0; x < map[y].split("").length; x++){
            if(map[y][x] == "1"){
                ...
                player = new component(x*50+50,y*50+50,"./sprites/player.png",40,40,baked_map)
                spawnpoints[0] = [x*50+50,y*50+50]
            }else if(map[y][x] == "e"){
                nemesis = new enemy(x*50+50,y*50+50,"./sprites/enemy.png",50,50)
                spawnpoints[1] = [x*50+50,y*50+50]
            }else if(map[y][x] == "n"){
                food.push(new object(x*50+50,y*50+50,"./sprites/apple.png",35,35))
    ...
    if(!isStarted){
        ...
        if((nemesis != null && player.crashWith(nemesis)) || player.y > window.innerHeight){
            ...
            if(levelInt == 3){
                ...
                milks -= 1
                foods = 0
                player = new component(spawnpoints[0][0],spawnpoints[0][1],"./sprites/player.png",40,40,baked_map)
                nemesis = new enemy(spawnpoints[1][0],spawnpoints[1][1],"./sprites/enemy.png",50,50) 
                food = []
                spawnpointsNuts.forEach(s => {
                    food.push(new object(s[0],s[1],"./sprites/apple.png",35,35))
                })
            } 
